import numpy as np


def generate_arcs(Teacher_lst, Period_lst, Day_lst, Classes_lst):
    idle_arcs = np.empty((Teacher_lst, Period_lst-1, Day_lst), dtype='U20') # a11 => string for 11 chrs
    dayoff_arcs = np.empty((Teacher_lst, 1, Day_lst), dtype='U20')
    for teacher in range(Teacher_lst):
        for day in range(Day_lst):
            # b notation -> b_TA, T = Teacher, A = Day
            # Note that the number of day off arcs is not # Days - 1, since the number of day nodes is actually # Day + 1
            # The number of acrs is # nodes - 1, thus, # Days = # Dayoff arcs.
            dayoff_arcs[teacher, 0, day] = "b_{}_{}".format(teacher + 1, day + 1)
            for period in range(Period_lst-1):
                # w notation -> w_TAC, T = Teacher, A = Day, C = From Period
                idle_arcs[teacher, period, day] = "w_{}_{}_{}".format(teacher + 1, day + 1, period + 1)

    interior_arcs = np.empty((Teacher_lst, (Period_lst*2-3)*Classes_lst, Day_lst), dtype='U20')
    for teacher in range(Teacher_lst):
        for day in range(Day_lst):
            for clas in range(Classes_lst):
                for i in range(Period_lst-1):
                    # x notation -> x_TABCD, T = Teacher, A = Day, B = Class, C = From Period, D = To Period
                    interior_arcs[teacher, i + (Period_lst*2-3)*clas, day] = 'x_{}_{}_{}_{}_{}'.format(teacher + 1, day + 1, clas + 1,
                                                                                                            i + 1, i + 2)
                    # print 'interior \t\t', interior_arcs[teacher][i][day]
                for k in range(Period_lst-2):
                    interior_arcs[teacher, k + Period_lst-1 + (Period_lst*2-3)*clas, day] = 'x_{}_{}_{}_{}_{}'.format(teacher + 1, day + 1, clas + 1, k + 1, k + 3)
                    # print 'exterior \t\t', interior_arcs[teacher][k+(len(Period_lst)-1)][day]

    # Need to distinguish between initializing arcs (e.g. source to inflow node), entering arcs (e.g. inflow node to period node), leaving arcs (e.g. period node to following day "source")
    transition_arcs = np.empty((Teacher_lst, (2*Period_lst-1), Day_lst), dtype='U20')
    for teacher in range(Teacher_lst):
        for day in range(Day_lst):
            for i in range(2*Period_lst-1):
                if i == 0:
                    # i notation -> i_TA, T = Teacher, A = Day (Initializing Arc)
                    transition_arcs[teacher, i, day] = "i_{}_{}".format(teacher + 1, day + 1)
                elif i < Period_lst:
                    # e notation -> s_TAD, T = Teacher, A = Day, D = To Period (Entering Arc)
                    transition_arcs[teacher, i, day] = "e_{}_{}_{}".format(teacher + 1, day + 1, i)
                else:
                    # l notation -> e_TAC, T = Teacher, A = Day, C = From Period (Leaving Arc)
                    transition_arcs[teacher][i][day] = "l_{}_{}_{}".format(teacher + 1, day + 1, i - Period_lst + 2)

    unsatisfied_var = np.empty((Teacher_lst, Classes_lst), dtype='U20')
    for teacher in range(Teacher_lst):
        for clas in range(Classes_lst):
            unsatisfied_var[teacher, clas] = 'g_{}_{}'.format(teacher + 1, clas + 1)
    return idle_arcs, dayoff_arcs, interior_arcs, transition_arcs, unsatisfied_var


def objective_function(Teacher_lst, unsatisfied, idle, interior, delta, omega, gamma):
    objective = ""
    for teacher in range(Teacher_lst):
        obj_teach = ''
        for unsatisfied_var in unsatisfied[teacher].flatten():
            obj_teach += "{}*{} + ".format(delta, unsatisfied_var)
        for idle_arc in idle[teacher].flatten():
            obj_teach += "{}*{} + ".format(omega, idle_arc)
        for interior_arc in interior[teacher].flatten():
            obj_teach += "{}*{} + ".format(gamma, interior_arc)
        objective += obj_teach
    return objective[:-3]


def c_flow_per_teacher(transition_arcs, idle_arcs, interior_arcs, dayoff_arcs, period_lst, class_lst, day_lst):
    neutral_eqs = []
    for i in range(len(day_lst)):
        for j in range(len(period_lst)):
            if j == (len(period_lst)-1):
                sum_node = "-1*{}".format(transition_arcs[-1][i])
                sum_node += " + {}".format(idle_arcs[j - 1][i])
            else:
                if j == 0:
                    sum_node = transition_arcs[1 + j][i]
                    # print(idle_arcs[j][i])
                    sum_node += " - {}".format(idle_arcs[j][i])
                else:
                    sum_node = "{} - {} - {} + {}".format(transition_arcs[1 + j][i], transition_arcs[len(period_lst) - 1 + j, i],
                                                          idle_arcs[j][i], idle_arcs[j - 1][i])
            for k in range(len(class_lst)):
                index = k*(2*len(period_lst)-3) + j
                if j == 0:
                    sum_node += " - {} - {}".format(interior_arcs[index][i], interior_arcs[(len(period_lst) - 1) + index][i])
                    if k == (len(class_lst)-1):
                        sum_node += " = 0;"
                        neutral_eqs.append(sum_node)
                else:
                    index_old = k * (2 * len(period_lst) - 3) + (j - 1)
                    if j < ((len(period_lst)-1)-1):
                        if j < 2:
                            index_old_old = k*(2*len(period_lst)-3)
                            sum_node += " + {} - {} - {}".format(interior_arcs[index_old_old][i], interior_arcs[index][i],
                                                                 interior_arcs[(len(period_lst) - 1) + index][i])
                            if k == (len(class_lst) - 1):
                                sum_node += " = 0;"
                                neutral_eqs.append(sum_node)
                        else:
                            sum_node += " + {} + {} - {} - {}".format(interior_arcs[index_old][i],
                                                                      interior_arcs[(len(period_lst)-1)+index_old][i],
                                                                      interior_arcs[index][i], interior_arcs[(len(period_lst)-1)+index][i])
                            if k == (len(class_lst) - 1):
                                sum_node += " = 0;"
                                neutral_eqs.append(sum_node)
                    elif j == ((len(period_lst)-1)-1):
                        sum_node += " + {} + {} - {}".format(interior_arcs[index_old][i],
                                                             interior_arcs[(len(period_lst)-1)+index_old][i], interior_arcs[index][i])
                        if k == (len(class_lst) - 1):
                            sum_node += " = 0;"
                            neutral_eqs.append(sum_node)
                    else:
                        index_old_old = k * (2 * len(period_lst) - 3) + j - 2
                        sum_node += " + {} + {}".format(interior_arcs[index_old][i], interior_arcs[(len(period_lst) - 1) + index_old_old][i])
                        if k == (len(class_lst) - 1):
                            sum_node += " = 0;"
                            neutral_eqs.append(sum_node)

    day_start_nodes = []
    dayoff_nodes = []
    for i in range(len(day_lst)):
        inflow = "{}".format(dayoff_arcs[0][i])
        if i == 0:
            outflow = "-1*{} - {} = -1;".format(dayoff_arcs[0][0], transition_arcs[0][0])
            dayoff_nodes.append(outflow)
            start = "{}".format(transition_arcs[0][i])
            for j in range(len(period_lst)-1):
                if i == (len(day_lst) - 1):
                    inflow += " + {}".format(transition_arcs[-1 * (j + 1)][i])
                    if j == (len(period_lst)-2):
                        inflow += " = 1"
                        dayoff_nodes.append(inflow)
                else:
                    inflow += " + {}".format(transition_arcs[-1 * (j + 1)][i])
                    start += " - {}".format(transition_arcs[j+1][i])
                    if j == (len(period_lst)-2):
                        inflow += " - {} - {} = 0;".format(transition_arcs[0][i + 1], dayoff_arcs[0][i + 1])
                        start += " = 0;"
                        dayoff_nodes.append(inflow)
                        day_start_nodes.append(start)
        else:
            start = "{}".format(transition_arcs[0][i])
            for j in range(len(period_lst)-1):
                if i == (len(day_lst) - 1):
                    inflow += " + {}".format(transition_arcs[-1 * (j + 1)][i])
                    start += " - {}".format(transition_arcs[j + 1][i])
                    if j == (len(period_lst)-2):
                        inflow += " = 1;"
                        start += " = 0;"
                        dayoff_nodes.append(inflow)
                        day_start_nodes.append(start)
                else:
                    inflow += " + {}".format(transition_arcs[-1 * (j + 1)][i])
                    start += " - {}".format(transition_arcs[j + 1][i])
                    if j == (len(period_lst)-2):
                        inflow += " - {} - {} = 0;".format(transition_arcs[0][i + 1], dayoff_arcs[0][i + 1])
                        start += " = 0;"
                        dayoff_nodes.append(inflow)
                        day_start_nodes.append(start)
    return day_start_nodes, dayoff_nodes, neutral_eqs


def generate_constraint_2(day_lst, class_lst, period_lst, teacher_lst, interior_arcs):
    c_lst = []
    for day in range(day_lst):
        for clas in range(class_lst):
            for period in range((2*period_lst-3)):
                xt_sum = ''
                index = clas*(2*period_lst - 3) + period
                for teacher in range(teacher_lst):
                    if teacher == 0:
                        xt_sum = interior_arcs[teacher][index][day]
                    else:
                        xt_sum += " + {}".format(interior_arcs[teacher][index][day])
                        if teacher == (teacher_lst-1):
                            xt_sum += " <= 1;"
                            c_lst.append(xt_sum)
    return c_lst


def generate_duration_contraint(day_lst, interior_arcs, duration, daily_duration):

    def determine_sta(string):
        return int(string[-1]) - int(string[-3])

    weighted_day_eqs = []
    weighted_day_sums = []
    for i in range(len(day_lst)):
        w_day_eq = ''
        for j in range(len(interior_arcs)):
            if j == 0:
                weight = determine_sta(interior_arcs[j][i])
                w_day_eq = "{}*{}".format(weight,interior_arcs[j][i])
            else:
                weight = determine_sta(interior_arcs[j][i])
                w_day_eq += "+ {}*{}".format(weight, interior_arcs[j][i])
                if j == (len(interior_arcs) - 1):
                    w_day_sum = "{} = {};".format(w_day_eq, daily_duration)
                    weighted_day_eqs.append(w_day_eq)
                    weighted_day_sums.append(w_day_sum)
    total_sum = ''
    total_sum_lst = []
    for j in range(len(weighted_day_eqs)):
        if j == 0:
            total_sum = weighted_day_eqs[j]
        else:
            total_sum += " + {}".format(weighted_day_eqs[j])
            if j == len(weighted_day_eqs)-1:
                total_sum += "= {};".format(duration)
                total_sum_lst.append(total_sum)
    return total_sum_lst, weighted_day_sums


def generate_constraint_5(teacher_lst, day_lst, class_lst, period_lst, interior_arcs):
    x_lst = []
    for teacher in range(teacher_lst):
        for day in range(day_lst):
            for clas in range(class_lst):
                for period in range(period_lst-2):
                    index = period + clas*(2*period_lst - 3)
                    x_sum = "{} + {} = {};".format(interior_arcs[teacher][index][day], interior_arcs[teacher][index+1][day], 1)
                    x_lst.append(x_sum)
    return x_lst


def generate_unsatisfied_double_constraint(min_double, unsatisfied_var, interior_arcs, Teacher_lst, Day_lst, Classes_lst, Period_lst):
    neg_sum_double = []
    for teacher in range(Teacher_lst):
        for day in range(Day_lst):
            for clas in range(Classes_lst):
                sum = ''
                for i in range(Period_lst - 2):
                    sum += " - {}".format(interior_arcs[teacher, clas * (2 * Period_lst - 3) + i + Period_lst - 1, day])
                neg_sum_double.append(sum)

    constr_lst = []
    for teacher in range(Teacher_lst):
        for clas in range(Classes_lst):
            constr = '{}{} <= {};'.format(min_double, neg_sum_double[3*teacher + clas], unsatisfied_var[teacher, clas])
            constr_lst.append(constr)
    return constr_lst


def generate_minimum_working_days_constraint(teacher_lst, day_lst, transition, min_working_days):
    constr_lst = []
    for teacher in range(teacher_lst):
        constr = ''
        for day in range(day_lst - 1):
            constr += "{} + ".format(transition[teacher, 0, day])
        else:
            constr += "{} >= {};".format(transition[teacher, 0, day_lst - 1], min_working_days)
        constr_lst.append(constr)
    return constr_lst


def bin_const_per_teacher(transition_arcs, idle_arcs, interior_arcs, dayoff_arcs):
    # Organise by day (each line has all day vars)
    str_lst = []
    for i in range(dayoff_arcs.shape[1]):
        # for interior arcs
        sum_int = "bin"
        for j in range(interior_arcs.shape[0] - 1):
            sum_int += " {},".format(interior_arcs[j][i])
        else:
            sum_int += "{};".format(interior_arcs[interior_arcs.shape[0] - 1, i])

        # for transition arcs
        sum_trans = "bin"
        for j in range(transition_arcs.shape[0] - 1):
            sum_trans += " {},".format(transition_arcs[j][i])
        else:
            sum_trans += "{};".format(transition_arcs[transition_arcs.shape[0] - 1, i])

        # for idle arcs
        sum_idle = "bin"
        for j in range(idle_arcs.shape[0] - 1):
            sum_idle += " {},".format(idle_arcs[j][i])
        else:
            sum_idle += "{};".format(idle_arcs[idle_arcs.shape[0] - 1, i])

        # for idle arcs
        sum_do = "bin "
        for j in range(dayoff_arcs.shape[0] - 1):
            sum_do += " {},".format(dayoff_arcs[j][i])
        else:
            sum_do += "{};".format(dayoff_arcs[dayoff_arcs.shape[0] - 1, i])

        str = "{}\n{}\n{}\n{}".format(sum_int, sum_trans, sum_idle, sum_do)
        str_lst.append(str)
    return str_lst


number_teachers = 1
teachers = np.arange(1, number_teachers + 1, 1)

number_periods = 5
periods = np.arange(1, number_periods + 1, 1)

number_days = 3
days = np.arange(1, number_days + 1, 1)

number_classes = 2
classes = np.arange(1, number_classes + 1, 1)

min_num_double_lessons = 3

max_hrs = 12
max_hrs_per_day = 4

min_working_days = 2

delta = 1
omega = 3
gamma = 9

idle, dayoff, interior, transition, unsatisfied = generate_arcs(number_teachers, number_periods, number_days, number_classes)
#print(idle)
#print(dayoff)
#print(interior)
#print(transition)
#print(unsatisfied)

print('\nTotal number of variables: ', len(idle.flatten())
                                     + len(dayoff.flatten())
                                     + len(interior.flatten())
                                     + len(transition.flatten())
                                     + len(unsatisfied.flatten()))

print_objective = True
print_constraint_1 = True
print_constraint_2 = True
print_constraint_3 = True
print_constraint_4 = True
print_constraint_5 = True
print_constraint_6 = True
print_constraint_7 = True
print_constraint_8 = True

link_lst, b_lst, x_lst = c_flow_per_teacher(transition[0], idle[0], interior[0], dayoff[0], periods, classes, days)
objective = objective_function(number_teachers, unsatisfied, idle, interior, delta, omega, gamma)
total_duration_constraint, daily_duration_constraints = generate_duration_contraint(days, interior[0], max_hrs, max_hrs_per_day)

constraint_2 = generate_constraint_2(number_days, number_classes, number_periods, number_teachers, interior)

constraint_5 = generate_constraint_5(number_teachers, number_days, number_classes, number_periods, interior)

constraint_6 = generate_unsatisfied_double_constraint(min_num_double_lessons, unsatisfied,
                                                      interior, number_teachers, number_days, number_classes, number_periods)

constraint_7 = generate_minimum_working_days_constraint(number_teachers, number_days, transition, min_working_days)

if print_objective:
    print("\n/*--------------------- OBJECTIVE FUNCTION ---------------------*/")
    print("min: {};".format(objective))

if print_constraint_1:
    print("\n/*--------------------- CONSTRAINT 1 - Node inflow and outflow for a given teacher ---------------------*/")
    print("/* Day off Nodes: */")
    for i in range(len(b_lst)):
        print(b_lst[i])
    print("\n/* Day Start Nodes: */")
    for i in range(len(link_lst)):
        print(link_lst[i])
    print("\n/* Internal (Period) Nodes: */")
    for i in range(len(x_lst)):
        print(x_lst[i])

if print_constraint_2:
    print("\n/*--------------------- CONSTRAINT 2 - One Teacher per Arc ---------------------*/")
    for i in constraint_2:
        print(i)

if print_constraint_3:
    print("\n/*--------------------- CONSTRAINT 3 - Total Hours Constraint ---------------------*/")
    for i in range(len(total_duration_constraint)):
        print(total_duration_constraint[i])

if print_constraint_4:
    print("\n/*--------------------- CONSTRAINT 4 - Duration Constraints, per day ---------------------*/")
    for i in range(len(daily_duration_constraints)):
        print(daily_duration_constraints[i])

if print_constraint_5:
    print("\n/*--------------------- CONSTRAINT 5 - No Consecutive Single Arcs ---------------------*/")
    for i in constraint_5:
        print(i)

if print_constraint_6:
    print("\n/*--------------------- CONSTRAINT 6 - Non-negative Unsatisfied Double Lessons ---------------------*/")
    for i in constraint_6:
        print(i)

if print_constraint_7:
    print("\n/*--------------------- CONSTRAINT 7 - Minimum Working Days ---------------------*/")
    for i in constraint_7:
        print(i)

if print_constraint_8:
    print("\n/*--------------------- CONSTRAINT 8 - Binary Variable Constraints ---------------------*/")
    str_lst = bin_const_per_teacher(transition[0], idle[0], interior[0], dayoff[0])
    for i in range(len(str_lst)):
        print("\n/* Binary Constraints for day {}*/".format(i+1))
        print(str_lst[i])
