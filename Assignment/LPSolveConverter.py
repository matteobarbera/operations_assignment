...
def objective_function(Teacher_lst, unsatisfied, idle, transition, delta, omega, gamma):
    ...
    for teacher in range(Teacher_lst):
        ...
        for initializer_arc in transition[teacher, 0]:
...
def generate_constraint_34(day_lst, interior_arcs, duration, daily_duration):
...
def generate_constraint_6(min_double, unsatisfied_var, interior_arcs, Teacher_lst, Day_lst, Classes_lst, Period_lst):
    neg_sum_double = []
    for teacher in range(Teacher_lst):
        for day in range(Day_lst):
            for clas in range(Classes_lst):
...
number_periods = 2
number_periods += 1  # horrible hotfix
...
for i in range(number_teachers):
    links, bs, xs = generate_constraint_1(transition[i], idle[i], interior[i], dayoff[i], periods, classes, days)
...
for i in range(number_teachers):
    tot_duration, daily_duration = generate_constraint_34(days, interior[i], max_hrs, max_hrs_per_day)
